...
std::vector<KEY_CODE> get_pressed_keys_keyboard(const Uint8* keystate) {
	...
	for (int sc = 0; sc < numkeys; ++sc) {
		if (keystate[sc]) {
			KEY_CODE key = SDL2_scancode_to_enum(static_cast<SDL_Scancode>(sc));
			if (key != KEY_NONE)
...
std::vector<KEY_CODE> get_pressed_keys_mouse(Uint32 mouse_state) {
	...
	for (Uint8 btn = SDL_BUTTON_LEFT; btn <= SDL_BUTTON_X2; ++btn) {
		if (mouse_state) {
			KEY_CODE key = SDL2_mouse_to_enum(btn);
			if (key != KEY_NONE)
...
std::vector<KEY_CODE> get_pressed_keys_gamepad(SDL_GameController* gamepad) {
	...
	for (int b = SDL_CONTROLLER_BUTTON_A; b <= SDL_CONTROLLER_BUTTON_TOUCHPAD; ++b) {
		if (SDL_GameControllerGetButton(gamepad, static_cast<SDL_GameControllerButton>(b))) {
			KEY_CODE key = SDL2_gamepad_to_enum(static_cast<SDL_GameControllerButton>(b));
			if (key != KEY_NONE)
				pressed_buttons.push_back(key);
		}
	}
